...
const ExternalFacultyForm = () => {
  ...
  const handleSubjectChange = async (e) => {
    ...
    setSubjectId(selectedSubjectId); // Update the selected subject ID
    setFormData({ ...formData, faculty_subject: e.target.value });
  
    if (selectedSubjectId) {
      try {
        const response = await fetch(`http://localhost:3001/student-count/${selectedSubjectId}`);
        if (response.ok) {
          const { count } = await response.json();
          setFormData((prevFormData) => ({
            ...prevFormData,
            faculty_subject: e.target.value,
            student_count: count.toString(), // Update the student count automatically
          }));
        } else {
          console.error("Failed to fetch student count.");
          alert("Unable to fetch student count. Please try again.");
        }
      } catch (error) {
